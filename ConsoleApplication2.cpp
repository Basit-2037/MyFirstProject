...
int shipX = 700, shipY = 600, shipSpeed = 7;// Initial speed of spaceship
float shipScale = 0.4f; // spaceship size
float bulletScale = 0.1f; // bullet size
float bulletSpeed = 10.0f, enemyScale = 0.2f;
...
// Controlling Explosion
struct Explosion
{
    float x, y;// position of explosion
    float scale;// size of explosion
    int frames;
    bool active;
};
...
// Bullet Shooting from Spaceship and Creation
void ShootBullet(int shipX, int shipY, float shipScale, float bulletScale)
{
    if (multiShot > 0)
    {
        for (int dx = 0; dx <= 1; dx++) // 0 first bullet , 1 second bullet

        {
            for (int i = 0; i < MAX_BULLETS; i++)
            {
                if (!bullets[i].active)
                {
                    ...
                    bullets[i].x = shipX + (spaceship.width * shipScale / 2) - (bullet.width * bulletScale / 2) + dx * 15;// Firing locatin of bullet
...
// Speed of bullet vertically upward
...
// Enemy Texture and its position
...
// Bullet fired time and randomly bullet generation on random rows and cols
void EnemyShoot()
{
...
// bullets speed and bullet explosion with ship
void UpdateEnemyBullets(float speed, int shipX, int shipY, float shipScale)
{
    for (int i = 0; i < MAX_ENEMY_BULLETS; i++)
    {
        if (enemyBullets[i].active)
        {
            ...
            if (!level5)
            {
                enemyBullets[i].y += speed;// bullet direction is vertically downward
            }
            ...
            if (shipAlive)// enemy bullet collides with the ship
            {
                ...
                if (CheckCollisionRecs(shipRec, bulletRec))
                {
                    ...
                    for (int e = 0; e < MAX_EXPLOSIONS; e++) // explositin production
                    {
...
void DrawEnemyBullets(float bulletScale)
{
    for (int i = 0; i < MAX_ENEMY_BULLETS; i++)
    {
        if (enemyBullets[i].active)
        {
            Color bulletColor = RED; // Level 1 bullet color
...
// Check the number enemies in current level and unlock the next level
bool AllEnemiesDead()
{
    for (int r = 0; r < currentEnemyRows; r++)
        for (int c = 0; c < currentEnemyCols; c++)
...
// Explosion Control and its frames
...
// Spaceship bullet collide with the enemies Rectangle Collision Detection
    ...
    for (int i = 0; i < MAX_BULLETS; i++)
    {
        if (bullets[i].active)
        {
            Rectangle bulletRec = { bullets[i].x, bullets[i].y,
                                    bullet.width * bulletScale, bullet.height * bulletScale };// Hint for Collision Detection
            for (int row = 0; row < currentEnemyRows; row++)
            {
                for (int col = 0; col < currentEnemyCols; col++)
                {
                    if (enemyAlive[row][col])
                    {
                        ...
                        if (CheckCollisionRecs(bulletRec, enemyRec))
                        {
                            ...
                            if (!powerActive && rand() % (level2 ? 2 : 4) == 0) // ration of Chance to get power
                            {
...
// Enemy collides with ship different cases
    ...
    for (int i = 0; i < MAX_TOKENS; i++)
    {
        if (tokens[i].active && !tokens[i].collected)
        {
            ...
            if (CheckCollisionRecs(shipRec, tokenRec))
            {
                ...
                case 3: speedBoostTimer = 60 * 5; speedBoost = 60 * 5; break;// advantage
...
// Boss Description like Enterace 
    ...
    if (bossFireCooldown <= 0 && bossAlive)
    {
        for (int i = 0; i < MAX_ENEMY_BULLETS; i++)
        {
            if (!enemyBullets[i].active)
            {
                ...
                float bx = bossX + (boss.width * bossScale) / 2;// boss X position
                float by = bossY + (boss.height * bossScale);// boss Y position

                enemyBullets[i].x = bx;// enemy bullet with respect to x coordinates of boss
                enemyBullets[i].y = by;// enemy bullet with respect to y coordinates of boss

                float tx = shipX + (spaceship.width * shipScale) / 2;// bullet is controlled with respect to x cordinates of ship
                float ty = shipY + (spaceship.height * shipScale) / 2;// bullet is controlled with respect to y cordinates of ship

                float vx = tx - enemyBullets[i].x;// vector from bullet to ship
                float vy = ty - enemyBullets[i].y;
                float L = sqrt(vx * vx + vy * vy);// L = length of this vector
...
// Update bullet on each frame normal level ad boss level
